  ...
  public DataObject<Number> evaluate(Event evt) throws EvaluationUndecidableException {
    try {
      ...
      for (int i = 1; i < this.getParameters().size(); i++) {
        ...
        evalResult = this.getParameters().get(i).evaluate(evt);

        if (String.class.isAssignableFrom(evalResult.getValue().getClass())) {
          result = result.subtract(new BigDecimal((String)evalResult.getValue()));

        } else if (Integer.class.isAssignableFrom(evalResult.getValue().getClass())) {
          result = result.subtract(BigDecimal.valueOf((Integer)evalResult.getValue()));

        } else if (Long.class.isAssignableFrom(evalResult.getValue().getClass())) {
          result = result.subtract(BigDecimal.valueOf((Long)evalResult.getValue()));

        } else if (Float.class.isAssignableFrom(evalResult.getValue().getClass())) {
          result = result.subtract(BigDecimal.valueOf((Float)evalResult.getValue()));

        } else if (Double.class.isAssignableFrom(evalResult.getValue().getClass())) {
          result = result.subtract(BigDecimal.valueOf((Double)evalResult.getValue()));
      ...
      throw new EvaluationUndecidableException("Error during subtraction", e);
    }
  }
}
