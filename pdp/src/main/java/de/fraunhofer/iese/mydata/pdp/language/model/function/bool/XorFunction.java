 ...
 */
@XmlRootElement(namespace = "http://www.mydata-control.de/4.0/mydataLanguage", name = "xor")
public class XorFunction extends Function {
  ...
  public DataObject<Boolean> evaluate(Event evt) throws EvaluationUndecidableException {
    if (this.getParameters() == null || this.getParameters().isEmpty()) {
      throw new EvaluationUndecidableException("Cannot evaluate less than one argument for XOR");
    }

    boolean result = this.evaluateParameter(evt, this.getParameters().get(0));

    if (this.getParameters().size() > 1) {
      for (int i = 1; i < this.getParameters().size(); i++) {
        result ^= this.evaluateParameter(evt, this.getParameters().get(i));
      }
    }
  ...
  private boolean evaluateParameter(Event evt, IFunction op) throws EvaluationUndecidableException {
