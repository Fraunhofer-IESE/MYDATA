...
public class MultFunction extends Function {
    ...
    try {
      ...
      for (final IFunction op : this.getParameters()) {
        ...
        if (String.class.isAssignableFrom(evalResult.getValue().getClass())) {
          result = result.multiply(new BigDecimal((String)evalResult.getValue()));

        } else if (Integer.class.isAssignableFrom(evalResult.getValue().getClass())) {
          result = result.multiply(BigDecimal.valueOf((Integer)evalResult.getValue()));

        } else if (Long.class.isAssignableFrom(evalResult.getValue().getClass())) {
          result = result.multiply(BigDecimal.valueOf((Long)evalResult.getValue()));

        } else if (Float.class.isAssignableFrom(evalResult.getValue().getClass())) {
          result = result.multiply(BigDecimal.valueOf((Float)evalResult.getValue()));

        } else if (Double.class.isAssignableFrom(evalResult.getValue().getClass())) {
          result = result.multiply(BigDecimal.valueOf((Double)evalResult.getValue()));
      ...
      throw new EvaluationUndecidableException("Error during multiplication", e);
    }
  }
