...
/**
 * The Class PmpRestConnector_ConstructorsTest.
 */
public class PmpRestConnector_ConstructorsTest {
  ...
  public void testConstructorAndUrisNULL() {
    try {
      new PmpRestConnector((String)null);
      Assertions.fail("No exception has been thrown. Error in PmpRestConnector(string)");
  ...
  public void testConstructorAndUrisWrongFormat() {
    try {
      new PmpRestConnector("demo:demo://demo:demo");
      Assertions.fail("No exception has been thrown. Error in PmpRestConnector(string)");
    } catch (final Exception e) {
  ...
  public void testConstructorAndUrisAsString() {
    try {
      final PmpRestConnector connector = new PmpRestConnector(Constants.BASE_URL);
      this.assertValues(connector);
    } catch (final Exception e) {
      Assertions.fail("Exception has been thrown. Error in PmpRestConnector(string): " + e.getMessage());
    }
  }
  ...
  public void testConstructorAndUrisAsUri() {
    try {
      ...
      final PmpRestConnector connector = new PmpRestConnector(uri);
      this.assertValues(connector);
    } catch (final Exception e) {
      Assertions.fail("Exception has been thrown. Error in PmpRestConnector(string): " + e.getMessage());
    }
  ...
  public void testConstructorAndUrisPlain() {
    try {
      final PmpRestConnector connector = new PmpRestConnector(Constants.SCHEME, Constants.HOST, Constants.PORT, Constants.NAME);
      this.assertValues(connector);
    } catch (final Exception e) {
      Assertions.fail("Exception has been thrown. Error in PmpRestConnector(string): " + e.getMessage());
    }
   ...
   */
  private void assertValues(PmpRestConnector connector) {
