...
/**
 * The Class PipRestConnector_ConstructorsTest.
 */
public class PipRestConnector_ConstructorsTest {
  ...
  public void testConstructorAndUrisNULL() {
    try {
      new PipRestConnector((String)null);
      Assertions.fail("No exception has been thrown. Error in PipRestConnector(string)");
  ...
  public void testConstructorAndUrisWrongFormat() {
    try {
      new PipRestConnector("demo:demo://demo:demo");
      Assertions.fail("No exception has been thrown. Error in PipRestConnector(string)");
    } catch (final Exception e) {
  ...
  public void testConstructorAndUrisAsString() {
    try {
      final PipRestConnector connector = new PipRestConnector(Constants.BASE_URL);
      this.assertValues(connector);
    } catch (final Exception e) {
      Assertions.fail("Exception has been thrown. Error in PipRestConnector(string): " + e.getMessage());
    }
  }
  ...
  public void testConstructorAndUrisAsUri() {
    try {
      ...
      final PipRestConnector connector = new PipRestConnector(uri);
      this.assertValues(connector);
    } catch (final Exception e) {
      Assertions.fail("Exception has been thrown. Error in PipRestConnector(string): " + e.getMessage());
    }
  ...
  public void testConstructorAndUrisPlain() {
    try {
      final PipRestConnector connector = new PipRestConnector(Constants.SCHEME, Constants.HOST, Constants.PORT, Constants.NAME);
      this.assertValues(connector);
    } catch (final Exception e) {
      Assertions.fail("Exception has been thrown. Error in PipRestConnector(string): " + e.getMessage());
    }
   ...
   */
  private void assertValues(PipRestConnector connector) {
