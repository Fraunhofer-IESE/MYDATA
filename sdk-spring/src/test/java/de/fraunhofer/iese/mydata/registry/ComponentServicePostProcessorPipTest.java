...
public class ComponentServicePostProcessorPipTest {

  private static final String BEAN_NAME_1 = "bean1";

  private final DummyPipService service = new DummyPipService();
  ...
  public void prepareTestCandidate() {
    this.testCandidate = new ComponentServicePostProcessor(ComponentType.PIP, TestService.class,
        this.myDataEnvironment, "http://localhost:8081", null, null, this.restExposeHelper);
  ...
  public void givenWeAreLocal_noBeanWithAnnotationExists() throws InvalidEntityException,
    ...
    Mockito.verify(this.myDataEnvironment, times(0)).registerLocalPip(any(ComponentId.class),
        any());
  }
  ...
  @Test
  public void givenWeAreLocal_whenRegistrationFails_thenNoException() throws Exception {
    doReturn(OperationalMode.LOCAL).when(this.myDataEnvironment).getOperationalMode();
    this.returnOneServiceBean(this.service);
    when(this.myDataEnvironment.registerLocalPip(ArgumentMatchers.any(ComponentId.class),
        ArgumentMatchers.any())).thenThrow(ResourceUpdateException.class);
    this.testCandidate.onApplicationEvent(new ContextRefreshedEvent(this.context));
    verify(this.myDataEnvironment).registerLocalPip(ArgumentMatchers.any(ComponentId.class),
  ...
  public void givenWeAreCloud_whenServiceHasActionMethods_registrationShouldHappen()
    ...
    this.returnOneServiceBean(this.service);
    this.testCandidate.onApplicationEvent(new ContextRefreshedEvent(this.context));
    verify(this.myDataEnvironment, times(0)).registerLocalPip(
        ArgumentMatchers.any(ComponentId.class), ArgumentMatchers.any(Object.class));
    verify(this.myDataEnvironment, times(1)).registerManagedPip(
        new ComponentId("urn:component:test:pip:1234"), this.service,
        Collections.singletonList(URI.create("http://localhost:8081/pip/somePath")));
    verify(this.restExposeHelper, times(1)).registerPipComponentIdToPath("somePath",
        new ComponentId("urn:component:test:pip:1234"));
  }
